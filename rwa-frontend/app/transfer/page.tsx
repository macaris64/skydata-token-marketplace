...
export default function TransferPage() {
  ...
  const [recipientCompliance, setRecipientCompliance] = useState<boolean>(false);
  ...
  useEffect(() => {
    if (recipient) {
      ...
      if (valid) {
        ...
        setRecipientCompliance(true);
      } else {
        setRecipientCompliance(false);
      }
    } else {
      setRecipientCompliance(false);
    }
  ...
  const updateTransferData = (field: string, value: string) => {
    if (field === 'recipient') {
      setRecipient(value);
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  value={recipient}
                  onChange={(e) => updateTransferData('recipient', e.target.value)}
                  className={`font-mono ${
                    recipient && !recipientCompliance ? 'border-red-500' : ''
                  }`}
                />
                {recipient && !recipientCompliance && (
